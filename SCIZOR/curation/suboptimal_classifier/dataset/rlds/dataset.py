...
from curation.suboptimal_classifier.dataset.rlds import obs_transforms, traj_transforms
from curation.suboptimal_classifier.dataset.rlds.utils import goal_relabeling, task_augmentation
from curation.suboptimal_classifier.dataset.rlds.utils.data_utils import (
    allocate_threads,
...
from curation.suboptimal_classifier.dataset.rlds.utils.spec import ModuleSpec
import pickle
import os
from curation.suboptimal_classifier.dataset.rlds.oxe.oxe_dataset_configs import OXE_DATASET_CONTROL_FREQUENCY
...
def apply_frame_transforms(
    ...
    *,
    ...
    def apply_obs_transform(fn: Callable[[dict], dict], frame: dict) -> dict:
        ...
        if len(tf.shape(frame["task"]["language_instruction"])) == 0:
            frame["task"] = fn(frame["task"])
        else: 
            frame["task"] = dl.vmap(fn)(frame["task"])
...
def apply_dedup_curation_mask(
    ...
    **kwargs,
    ...
    for eps, prob in zip(curation_eps, curation_prob):
        assert f'eps={eps:g}' in current_dataset_curation_df.columns, f"eps={eps:g} not in curation_df, available columns are {current_dataset_curation_df.columns}"
        current_curation_df = current_dataset_curation_df[f'eps={eps:g}']
...
def add_subop_score(dataset, dataset_name, subop_mask_path):
    ...
    return dataset_with_score, scores
    ...
    def filter_fn(frame):
        ...
        if subop_score_path:
            if subop_sampling:
                ...
                to_curate = tf.logical_or(to_curate, frame['suboptimal_score'] > thres)
    ...
    *,
    ...
    def restructure(traj):
        ...
        if curation_args.get("load_delta_gripping_act", False) and train:
            ...
            restruct_traj['observation']['delta_gripping'] = tf.concat([tf.zeros((1,), dtype=tf.float32), restruct_traj['observation']['delta_gripping']], axis=0)
            
        # if curation_args.get("load_future_image", False) and train:
        #     freq = OXE_DATASET_CONTROL_FREQUENCY[name]
        #     time_gap = curation_args.get("future_image_time_gap", 2.0)
        #     positive = tf.random.uniform([traj_len], minval=0, maxval=1, dtype=tf.float32) > 0.5
        #     restruct_traj['task']['timestamp'] = tf.cast(tf.where(positive, time_gap, 0) * freq, tf.int32) + tf.range(traj_len)
        #     restruct_traj['task']['timestamp'] += tf.random.uniform([traj_len], minval=-freq//2, maxval=freq//2+1, dtype=tf.int32)
        #     restruct_traj['task']['timestamp'] = tf.clip_by_value(restruct_traj['task']['timestamp'], 0, traj_len)
        #     restruct_traj['task']['timestamp'] = tf.cast(restruct_traj['task']['timestamp'], tf.int32)
        #     restruct_traj['task']['delta_time'] = tf.cast(restruct_traj['task']['timestamp'] - tf.range(traj_len), tf.float32) / freq
    ...
    if curation_args.get('curation_df_path', None) is not None and train:
        ...
        if curation_args.get("curation_rebalance", False):
            dataset_statistics['num_transitions'] -= curate_size
        logging.info(f"Deduplication curate transitions:{curate_size}. Remaining transitions:{dataset_statistics['num_transitions']}. Keeping ratio:{dataset_statistics['num_transitions']/(dataset_statistics['num_transitions']+curate_size)}")
    ...
    if curation_args.get('subop_score_path', False) and train:
        dataset, scores = add_subop_score(dataset, name, curation_args.get('subop_score_path'))
        if scores is not None:
            curated_size = np.sum(scores > curation_args.get('subop_score_thres', 1.0))
            if curation_args.get("curation_rebalance", False):
                dataset_statistics['num_transitions'] -= curate_size
            logging.info(f"Suboptimal score curate transitions:{curated_size}. Remaining transitions:{dataset_statistics['num_transitions']}. Keeping ratio:{dataset_statistics['num_transitions']/(dataset_statistics['num_transitions']+curated_size)}")
    if curation_args.get('load_delta_proprio', False):
...
def make_single_dataset(
    ...
    *,
    ...
    dataset = apply_trajectory_transforms(dataset, dataset_kwargs["name"], **traj_transform_kwargs, train=train)
