...
class DenDedupJob(submitit.helpers.Checkpointable):
    """
    - Each SLURMJob will run DenDedup on number of clusters and save dataframe with which examples to keep from each cluster.
    ...
    def dendedup(self, cluster, cluster_reps, device):
        st = time.time()
        ...
        if self.args.sim_metric == "cosine":
            ...
            pair_w_sim_matrix.fill_diagonal_(-torch.inf) 
        ...
        ## -- if use kde, calculate density for each point
        # bandwidth = 1.06 * (cluster_reps.shape[0] ** (-1/5))
        # print(f"Bandwidth: {bandwidth}")
        if self.args.sim_metric == "cosine":
            # kernel_values = torch.exp(-(1-pair_w_sim_matrix)**2 / (2 * bandwidth ** 2))
            kernel_values = 1/((1-pair_w_sim_matrix)**2 + 1e-5)
        else:
            # kernel_values = torch.exp(-pair_w_sim_matrix**2 / (2 * bandwidth ** 2))
            kernel_values = 1/(pair_w_sim_matrix**2 + 1e-5)
        del cluster_reps
        
        # density = kernel_values.mean(dim=1)
        # sum top k kernel values
        k = 20
        density, _ = torch.topk(kernel_values, k=k, dim=1)
        density = density.mean(dim=1)
        # M = density / density.sum()
        M = density
        M = M.cpu()
    ...
    def _process_shard(self, start_cluster: int, end_cluster: int):
        ...
        for cluster_id in tqdm(range(start_cluster, end_cluster)):
            ...
            M, idx = self.dendedup(cluster_i, cluster_reps, self.args.device)
            ...
            points_to_remove_df["density"] = M.numpy()

            for eps in self.args.eps_list:
                ## initialize eps_points_to_remove as False
                eps_points_to_remove = torch.zeros(M.shape[0], dtype=bool)
                # sample eps percentage of points and mark idx as True
                if int(M.shape[0] * eps) > 0:
                    sampled_indices = torch.multinomial(M, num_samples=int(M.shape[0] * eps), replacement=False)
                    eps_points_to_remove[sampled_indices] = True
                points_to_remove_df[f"eps={eps}"] = eps_points_to_remove.numpy()
...
def launch(args):
    ...
    ## -- SUBMIT
    submitit_path = f"{args.save_folder}/dendedup-jobs/%j"
    ...
    with executor.batch():
        for job_start_cluster in range(0, args.num_clusters, args.clusters_per_job):
            exp = DenDedupJob(args, job_start_cluster)
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--bandwidth", type=float, default=0.5, help="Bandwidth for KDE")
