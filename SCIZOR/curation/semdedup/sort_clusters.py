...
def assign_and_sort_clusters(
    ...
    nearest_cent = np.load(nearest_cent_file_loc).squeeze()
    dist_to_cent = np.load(dist_to_cent_file_loc).squeeze()
    paths_list = pd.Series(paths_list)
    nearest_cent = pd.Series(nearest_cent)
    dist_to_cent = pd.Series(dist_to_cent)
...
def rank_within_cluster(
    ...
    for cluster_c in tqdm(cluster_ids):
        ...
        if not os.path.exists(os.path.dirname(sorted_cluster_file_path)):
            os.makedirs(os.path.dirname(sorted_cluster_file_path))
...
class SLURMJob(submitit.helpers.Checkpointable):
    ...
    def _encode_shard(self, args):
        ...
        metadata_path = pathlib.Path(args.save_folder, "metadata.pkl")
        with open(metadata_path, "rb") as f:
            loaded_metadata = pickle.load(f)

        loader = EmbeddingLoader(args.emb_memory_folder, loaded_metadata["modalities"])
        normalization = args.Kmeans_with_cos_dist
        data, metadata = loader.load_embeddings(normalization=normalization)
        assert metadata['sequence'] == loaded_metadata['sequence'], "Metadata Mismatch, the embedding components are not the same"
        
        paths_list = metadata["id"]["all"]
...
def launch_jobs(args):
    ...
    args.sorted_clusters_file_loc = os.path.join(args.save_folder, "sorted_clusters")
    ...
    for job in jobs:
        print(f"Submitted job_id: {job.job_id} for sorting clusters")
