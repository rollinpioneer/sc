...
def main(args):
    ...
    with open(
        pathlib.Path(args.save_folder, "clustering_params.txt"), "w"
    ) as fout:
        pprint.pprint(params, fout)

    ## -- Load clustering parameters
    seed = params["seed"]
    emb_memory_folder = params[
        "emb_memory_folder"
    ...
    # )
    embedding_loader = EmbeddingLoader(emb_memory_folder, modality)
    normalization = Kmeans_with_cos_dist
    data, metadata = embedding_loader.load_embeddings(normalization=normalization)
    
    # save metadata
    with open(
        pathlib.Path(save_folder, "metadata.pkl"), "wb"
    ) as fout:
...
if __name__ == "__main__":
    ...
    elif os.path.exists(os.path.join(params["save_folder"], params["timestamp"])):
        timestr = params["timestamp"] + '-' + time.strftime("%Y%m%d-%H%M%S")
    else:
        timestr = params["timestamp"]
    params["timestamp"] = timestr
    with open(args.config_file, "w") as y_file:
        yaml.dump(params, y_file)
        
    args.save_folder = os.path.join(params["save_folder"], timestr)
    ...
    print(f"Submitted job_id: {job.job_id} for clustering")
    result = job.result()