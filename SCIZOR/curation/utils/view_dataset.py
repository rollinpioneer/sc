...
class TrajLoader:
    ...
    def visualize(self, num:int, save_dir:str='./video', save_instruction:bool=False):
        """ 
        ...
        transform = OXE_STANDARDIZATION_TRANSFORMS[self.name]
        for i, demo in enumerate(self.dataset):
            images = tf.map_fn(tf.image.decode_jpeg, demo['steps']['observation'][obs_key], dtype=tf.uint8).numpy()
            save_video(save_dir, f"{i}", images, demo, save_instruction)
            if i == num:
                break