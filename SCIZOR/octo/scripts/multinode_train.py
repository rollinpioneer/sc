...
print("num_processes", os.environ['SLURM_NTASKS'], "process_id", os.environ['SLURM_PROCID'])

MASTER_ADDR = os.environ.get('MASTER_ADDR')
MASTER_PORT = os.environ.get('MASTER_PORT')
main_node_ip = MASTER_ADDR
print("Main_Node_IP_address", main_node_ip)

jax.distributed.initialize(
    coordinator_address=f'{main_node_ip}:{MASTER_PORT}',  # Use the coordinator node's IP
    num_processes=int(os.environ['SLURM_NTASKS']),
    process_id=int(os.environ['SLURM_PROCID']),
    local_device_ids=os.environ['CUDA_VISIBLE_DEVICES'].split(',')
)
print(f"Process {jax.process_index()} of {jax.process_count()}, with {len(jax.local_devices())} devices per process")
...
# from octo.utils.multiprocess_dataloading import multiprocessing_data_loader, deserialize_nested_dict
from multiprocessing import Process, Pipe
import dlimp as dl
from octo.utils.dali_transform import dali_obs_transform_pipeline, ExternIterator, Process_Dataset, test_pipeline, FinalDataset, InterleaveDaliDataset, get_timer
...
def main(_):
    ...
    if jax.process_index() == 0:
        codebase_directory = osp.abspath(osp.join(osp.dirname(octo.model.__file__), "../.."))
    ...
    if FLAGS.config.curation.get("save_folder", None) is not None:
        FLAGS.config.curation.curation_df_path = osp.join(FLAGS.config.curation.save_folder, "semdedup/dataframes/concat_df.pkl")
        config_path = osp.join(FLAGS.config.curation.save_folder, "clustering_params.txt")
        with open(config_path, "r") as f:
            semdedup_config = eval(f.read())
            FLAGS.config.curation.semdedup_config.update(semdedup_config)
    FLAGS.config.curation.seed = FLAGS.config.seed
    ...
    def transform_fn(batch):
        with timer("process batch"):
            batch = process_batch(batch)
        with timer("shard"):
            batch = shard(batch)
        return batch
    if FLAGS.config.dali_kwargs.get("use_dali", False):
        make_interleaved_dataset(**FLAGS.config.dataset_kwargs, train=True, curation_args=FLAGS.config.curation, use_dali=True)  # Just to collect the log for dataset creation
        dali_iterator = ExternIterator(config=FLAGS.config)
        dataset_statistics = dali_iterator.dataset_statistics
        
        train_data_iter = InterleaveDaliDataset(
            FLAGS.config,
            source = dali_iterator,
            **FLAGS.config.dali_kwargs,
            transform_fn = transform_fn,
        )
    else:
        train_dataset = make_interleaved_dataset(**FLAGS.config.dataset_kwargs, train=True, curation_args=FLAGS.config.curation, use_dali=False)
        train_data_iter = map(
            transform_fn,
            train_dataset.iterator(prefetch=FLAGS.config.prefetch_num_batches)
        )
    ...
    elif FLAGS.config.resume_path is not None:
        resume_save_callback = SaveCallback(FLAGS.config.resume_path)
        train_state = resume_save_callback.state_checkpointer.restore(
            resume_save_callback.state_checkpointer.latest_step(), items=train_state
        )
    ...
    def loss_fn(params, batch, rng, train=True):
        ...
        action_loss, action_metrics = bound_module.heads["action"].loss(
            ...
            batch["pivot_weight"] if "pivot_weight" in batch else None,
            train=train,
        )
    ...
    def train_step(state: TrainState, batch: Data):
        ...
        step = state.step
        if 'action_pivot' in batch:
            theta = (step - FLAGS.config.curation.pivot_start_step) / (FLAGS.config.curation.pivot_end_step - FLAGS.config.curation.pivot_start_step)
            theta = jax.numpy.clip(theta, 0, 1)
            action_pivot_weight = (1-theta) * FLAGS.config.curation.pivot_start_weight + theta * FLAGS.config.curation.pivot_end_weight
            batch['pivot_weight'] = jax.numpy.where(batch['action_pivot'], action_pivot_weight, 1.0)
        ...
        if 'pivot_weight' in batch:
            info.update({"pivot_weight_variance": jax.numpy.var(batch['pivot_weight'])})
    ...
    if "rollout_kwargs" in FLAGS.config:
        ...
        rollout_callback = RolloutVisualizationCallback(
            text_processor=text_processor,
            action_proprio_metadata=dataset_statistics[dataset_name],
            **rollout_kwargs,
        )
    else:
    ...
    for i in tqdm.tqdm(
        ...
        if (i + 1) % FLAGS.config.log_interval == 0:
            ...
            wandb_log(
                {"training": update_info, "timer": timer.get_average_times(), "dali_timer": dali_timer.get_average_times()},
                step=i + 1,
            )
