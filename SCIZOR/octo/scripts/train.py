...
# num_cpu_to_use_for_dali = int(num_cpus//12)
# os.environ["DALI_AFFINITY_MASK"]=','.join([str(i%num_cpu_to_use_for_dali) for i in range(1024)])
...
def main(_):
    ...
    def loss_fn(params, batch, rng, train=True):
        ...
        action_loss, action_metrics = bound_module.heads["action"].loss(
            ...
            batch["act_score"] if "act_score" in batch else None,
            train=train,
        )
    ...
    for i in tqdm.tqdm(
        ...
        if (i + 1) % FLAGS.config.save_interval == 0 and (i + 1) >= FLAGS.config.save_start:
