...
class ScoreActor(nn.Module):
    ...
    act_score_encoder: Optional[nn.Module] = None

    def __call__(self, obs_enc, actions, time, act_score: Optional[jax.Array] = None, train=False):
        """
        Args:
        ...
        if self.act_score_encoder is not None:
            if act_score is None:
                act_score = jnp.zeros((cond_enc.shape[:-1] + (4,))) # hardcode action_horizon for now
            act_score_emb = self.act_score_encoder(act_score)
            cond_enc = jnp.concatenate([cond_enc, act_score_emb], axis=-1)
...
def create_diffusion_model(
    ...
    if act_score_cond:
        act_score_encoder = MLP((act_score_dim * 2, act_score_dim)) # hardcode window * action_horizon for now
        print("Condition on act_score")
    else:
        act_score_encoder = None
    return ScoreActor(
        ...
        act_score_encoder=act_score_encoder,
    )
