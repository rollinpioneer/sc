...
def continuous_loss(
    ...
    mask: ArrayLike,
    loss_type: str = "mse",
    pivot_weight: Optional[ArrayLike] = None,
    act_score: Optional[ArrayLike] = None,
) -> Array:
    """
    Args:
    ...
    if pivot_weight is not None:
        # normalize the pivot_weight
        pivot_weight = pivot_weight/pivot_weight.sum()*pivot_weight.size
        pivot_weight = pivot_weight[None, :, None, None]
        pivot_weight = jnp.broadcast_to(pivot_weight, loss.shape)
        # breakpoint()
        loss = loss * pivot_weight
...
class DiffusionActionHead(nn.Module):
    ...
    act_score_cond: bool = False
    act_score_dim: int = 16
    ...
    def setup(self):
        ...
        self.diffusion_model = create_diffusion_model(
            ...
            act_score_cond=self.act_score_cond,
            act_score_dim=self.act_score_dim,
        )
    ...
    def __call__(
        ...
        pred_eps = self.diffusion_model(embeddings, noisy_actions, time, act_score, train=train)
        return pred_eps
    ...
    def loss(
        ...
        pivot_weight: Optional[ArrayLike] = None,
        act_score: Optional[ArrayLike] = None,
        cond_on_act_score: bool = False,
        loss_reweight_on_act_score: bool = False,
        train: bool = True,
    ) -> Tuple[Array, Dict[str, Array]]:
        ...
        pred_eps = self(
            transformer_outputs, train=train, time=time, noisy_actions=noisy_actions, act_score=act_score[None, ...] if act_score is not None and cond_on_act_score else None
        )
        ...
        loss, metrics = continuous_loss(pred_eps, noise, mask, loss_type=self.loss_type, pivot_weight=pivot_weight, act_score=act_score if loss_reweight_on_act_score else None)
        # Sum over action dimension instead of averaging
        loss = loss * self.action_dim
        metrics["loss"] = metrics["loss"] * self.action_dim
        metrics["mse"] = metrics["mse"] * self.action_dim
