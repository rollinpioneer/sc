...
def chunk_act_obs(
    ...
    if 'act_score' in traj:
        traj['act_score'] = tf.gather(traj['act_score'], action_chunk_indices)
        traj['act_score'] = tf.gather(traj['act_score'], history_indices)
...
def filter_wrong_padding(traj: dict, window_size:int, action_horizon:int) -> dict:
    """Filters out trajectories where the padding is incorrect.

    This can happen if the trajectory was generated with the curation script and the curation chunking scheme
    """
    traj_len = tf.shape(traj["action"])[0]
    if not traj['is_first'][0] and traj_len > window_size:
        traj = tf.nest.map_structure(lambda x: x[window_size-1:], traj)
    if not traj['is_last'][-1] and traj_len > action_horizon:
        traj = tf.nest.map_structure(lambda x: x[:-(action_horizon-1)], traj)
    return traj
