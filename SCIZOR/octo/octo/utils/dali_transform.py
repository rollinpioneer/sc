...
def dataset_process_fn(config, child_conn):
    train_data = make_interleaved_dataset(**config.dataset_kwargs, train=True, curation_args=config.curation, use_dali=True)
...
class Process_Dataset():
    def __init__(self, config,):
        ...
        self.process = Process(target=dataset_process_fn, args=(config, self.child_conn), daemon=True)
...
class FinalDataset():
    def __init__(self, extern_dataset, process_pipeline, buffer_device=None):
        ...
        self.dali_iterator = DALIGenericIterator(self.process_pipeline, output_map=['primary', 'wrist', 'timestep_marker'])
...
class ExternIterator(object):
    def __init__(self, config, max_size=10):
        self.dataset = Process_Dataset(config)
        ...
        self.observation_image_keys = [key for key in self.example_batch['observation'] if key.startswith("image_")]
    ...
    def get_num_outputs(self):
        return len(self.task_image_keys) + len(self.observation_image_keys) * self.window_size
    ...
    def get_image_keys(self):
        ...
        # for key in self.task_image_keys:
        #     batch_to_transform[f"task.{key}.0"] = []
        ...
        self.image_keys = {'primary':[], 'wrist':[]}
        for key in self.task_image_keys:
            self.image_keys[key[6:]].append(f"task.{key}.0")
        for key in self.observation_image_keys:
            for i in range(self.window_size):
    ...
    def next(self):
        ...
        for key in self.task_image_keys:
            futures[f"task.{key}.0"] = convert_list_bytes_to_np_array.remote(frame['task'][key].tolist(), self.zero_pad_encoded)
        for key in self.observation_image_keys:
            for i in range(self.window_size):
                futures[f"observation.{key}.{i}"] = convert_list_bytes_to_np_array.remote(frame['observation'][key][:,i].tolist(), self.zero_pad_encoded)
        
        flattened_images = []
        flattened_mask = {'primary':[], 'wrist':[]}
        images = {'primary':[], 'wrist':[]}
        for key in self.task_image_keys:
            batch_to_transform[f"task.{key}.0"] = ray.get(futures[f"task.{key}.0"])
            # flattened_images.extend(batch_to_transform[f"task.{key}.0"])
            images[key[6:]].extend(batch_to_transform[f"task.{key}.0"])
            
        for key in self.observation_image_keys:
            for i in range(self.window_size):
                batch_to_transform[f"observation.{key}.{i}"] = ray.get(futures[f"observation.{key}.{i}"])
                # flattened_images.extend(batch_to_transform[f"observation.{key}.{i}"])
                images[key[6:]].extend(batch_to_transform[f"observation.{key}.{i}"])
                
        for key in self.task_image_keys:
            batch_to_transform[f"task.pad_mask_dict.{key}"] = np.split(np.array(frame['task']['pad_mask_dict'][key], dtype=np.uint8), self.batch_size, axis=0)
            flattened_mask[key[6:]].extend(batch_to_transform[f"task.pad_mask_dict.{key}"])
        ...
        # for key in self.task_image_keys:
        #     batch_to_transform[f"task.reduced_pad_mask_dict.{key}"] = [np.array(np.logical_or.reduce(frame['task']['pad_mask_dict'][key], axis=0), dtype=np.uint8)]*self.batch_size
        # for key in self.observation_image_keys:
        #     for i in range(self.window_size):
        #         batch_to_transform[f"observation.reduced_pad_mask_dict.{key}.{i}"] = [np.array(np.logical_or.reduce(frame['observation']['pad_mask_dict'][key][:,i], axis=0), dtype=np.uint8)]*self.batch_size
        # batch_to_transform['timestep'] = np.split(time_step_marker, self.batch_size, axis=0)
        
        batch_to_transform['timestep'] = np.split(time_step_marker, self.batch_size, axis=0) * len(self.image_keys['primary'])
        self.aug_rands = np.random.rand(self.batch_size, 10).astype(np.float32)
        self.aug_rands = np.split(self.aug_rands, self.batch_size)
        
        return images['primary'], images['wrist'], flattened_mask['primary'], flattened_mask['wrist'], batch_to_transform['timestep'], self.aug_rands * len(self.image_keys['primary'])
        # return flattened_images, flattened_mask, batch_to_transform['timestep']
        # return tuple(batch_to_transform.values())
    
    def __del__(self):
        ray.shutdown()

def augment_brightness(
    input, scale: list[float], rand:float
):
    # rand_shift = fn.random.uniform(range=(-scale[0], scale[0]))
    rand_shift = scale[0] * (rand - 0.5) * 2
    result = fn.brightness(input, brightness_shift=rand_shift)
    return result

def augment_contrast(
    input, scale: tuple[float, float], rand:float
):
    # rand_scale = fn.random.uniform(range=[scale[0], scale[1]])
    rand_scale = rand * (scale[1] - scale[0]) + scale[0]
...
def augment_random_resized_crop(
    input, ratio, scale, size, rand
):
    random_scale = rand[0] * (scale[1] - scale[0]) + scale[0]
    random_ratio = rand[1] * (ratio[1] - ratio[0]) + ratio[0]
    
    new_height = dali_math.clamp(dali_math.sqrt(random_scale / random_ratio), 0, 1)
    new_width = dali_math.clamp(dali_math.sqrt(random_scale * random_ratio), 0, 1)
    
    crop_h = new_height * size[0]
    crop_w = new_width * size[1]
    
    height_offsets = rand[2] * (1 - new_height)
    width_offsets = rand[3] * (1 - new_width)
    
    image = fn.crop(input, crop_w=crop_w, crop_h=crop_h, crop_pos_x=height_offsets, crop_pos_y=width_offsets)
    image = fn.resize(image, size=size)
    
    return image

@pipeline_def
def test_pipeline(image_augment_kwargs):
    files, label = fn.readers.file(file_root="Downloads/test_images")
...
def dali_obs_transform_pipeline(
    ...
    frame = fn.external_source(
        source=source, num_outputs=6, device='cpu', dtype=[types.UINT8]*5 + [types.FLOAT]
    )
    key_list = ["primary", "wrist"]
    aug_rands = frame[-1][0]
    for i,current_key in enumerate(key_list):
        # current_key = key_list[i]
        current_pad_mask = frame[i+num_outputs]
        ...
        for j,augment_op in enumerate(augment_arg['augment_order']):
            if augment_op in AUGMENT_OPS and augment_arg.get(augment_op) is not None:
                if augment_op == "random_resized_crop":
                    frame[i] = AUGMENT_OPS["random_resized_crop"](frame[i], **augment_arg["random_resized_crop"], size=resize_size[current_key], rand = aug_rands[-4:])
                    # continue
                else:
                    frame[i] = AUGMENT_OPS[augment_op](frame[i], augment_arg[augment_op], rand = aug_rands[j])
        frame[i] = frame[i] * current_pad_mask
        
    # the last element is timestep marker
    return tuple(frame[:num_outputs]) + (frame[-2],)
...
class InterleaveDaliDataset:
    def __init__(
        ...
        buffer_size: int = 2,
        buffer_device: int = None, # the last device
        use_dali: bool = True,
    ):
        assert device in ['cpu', 'mixed']
        self.config = config
