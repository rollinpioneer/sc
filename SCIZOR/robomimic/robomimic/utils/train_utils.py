...
def get_exp_dir(config, auto_remove_exp_dir=False):
    ...
    hdf5_filter_key = ""
    if config.train.hdf5_filter_key is not None:
        hdf5_filter_key = "_" + config.train.hdf5_filter_key
    seed = ""
    if config.train.seed is not None:
        seed += "_seed{}".format(config.train.seed)

    time_str = "{}{}{}".format(time_str, hdf5_filter_key, seed)
...
def dataset_factory(config, obs_keys, filter_by_attribute=None, dataset_path=None):
    ...
    ds_kwargs = dict(
        ...
        curation_args=config.train.get("curation", {}),
    )
...
def run_rollout(
    ...
    assert isinstance(env, EnvBase) or isinstance(env, SubprocVectorEnv)
    ...
    for step_i in iterator:
        ...
        if video_writer is not None:
            if video_count % video_skip == 0:
                video_imgs = env.render(mode="rgb_array", height=512, width=512)
                if batched:
...
def rollout_with_stats(
    ...
    for env_name, env in envs.items():
        ...
        for ep_i in iterator:
            ...
            if verbose:
                print("Episode {}, horizon={}, num_success={}".format(ep_i + 1, horizon[env_name], num_success))
...
def should_save_from_rollout_logs(
    ...
    for env_name in all_rollout_logs:
        ...
        if save_on_best_rollout_success_rate:
            ...
            epoch_ckpt_name += "_{}_success_{}".format(env_name, rollout_logs["Success_Rate"])
            should_save_ckpt = True
            ckpt_reason = "success"
